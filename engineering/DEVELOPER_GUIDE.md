...
This file is the internal working guide for maintainers and developers. The customer-facing root `README.md` and documents under `doc/` remain in Vietnamese.

## 1. Read before changing code

Start with:

1. `engineering/SESSION_HANDOFF.md` for the current working state.
2. `engineering/INDEX.md` for task-specific documentation routing.
3. `engineering/RULES.md` for repository rules.
...
Exact Vietnamese status values and user-visible labels are documented in the relevant files under `engineering/` and must not be translated in code or persisted data.
...
See `engineering/infra/TESTING.md`, `engineering/infra/TEST_RUNBOOK.md`, and `engineering/infra/TEST_MATRIX.md`.

## 10. Documentation policy

- engineering/developer-facing documentation is stored under `engineering/` and written in English.
...
- Update the relevant `engineering/` document when architecture, schema, workflow, permissions, or operational behavior changes.
