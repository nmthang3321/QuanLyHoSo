...
`AppDataService.Initialize` creates/repairs the schema, seeds the built-in administrator, standard areas, and default catalogs, normalizes legacy values, creates indexes, and starts the LAN listener in host mode. Normal production initialization leaves the record table empty; demo records are seeded only by the explicit sample-data flow. This refactor did not change schema SQL, table/column names, stored values, migrations, record numbering, filtering, sorting, or status rules.
...
- Initial result documents are generated by `InitialResultDocumentGenerator` using the existing `.docx` templates under `doc/templates/`.
