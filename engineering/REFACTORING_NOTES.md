...
The compatibility baseline is the pre-refactor implementation plus the screen/workflow notes under `engineering/`. The review traced startup, DataTemplates, navigation reset/reuse behavior, authentication, role checks, CRUD, record ownership, processing transitions, search/filter/sort/paging, export, attachments, notices/KPI, settings, backup/restore, update download, logging, LAN routing, database initialization, and shutdown.
